...
const getLinks = function (data) {
  ...
  for (let link of links) {
    if (last !== link.offset) data.ast.push({ type: 'text', text: wiki.slice(last, link.offset) })
    last = link.offset + link.raw.length
    delete link.offset
    data.ast.push({ ...link, type: `${link.type || 'internal'} link` })
  }
  if (last !== wiki.length) data.ast.push({ type: 'text', text: wiki.slice(last, wiki.length) })
